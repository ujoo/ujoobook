...
module.exports = function(app, fs)
{
    ...
    app.get('/etnews', function(req, res) {
        ...
        // 요청 세부 내용
        const req_url = 'http://www.paoin.com/service/Etnews/Default.aspx'
        // 요청 시작 받은값은 body
        axios.post(req_url, {
            'PaperDate': moment(art_date).format('YYYY-MM-DD')
            })
            .then(response => {
...
//    });
    ...
    app.get('/hani', function(req, res) {
		...
		case "h21" :
			mtit = "한겨례21";
			req_url = "http://h21pdf.hani.co.kr/PaperList.aspx";
			break;
		case "ecopdf" :
			mtit = "Insight";
			req_url = "http://ecopdf.hani.co.kr/PaperList.aspx";
			break;
		case "ndpdf" :
			mtit = "나들";
			req_url = "http://ndpdf.hani.co.kr/PaperList.aspx";
			break;
		default :
			mseq = "hani";
			req_url = "http://pdf.hani.co.kr/PaperList.aspx";
			mtit = "한겨례신문";
			formData = {'PaperDate': moment(art_date).format('YYYY-MM-DD')};
			break;
		}
        console.log(mtit + ' 정보 불러오는중...');
        console.log(req_url + formData);
        ...
        axios.post(req_url, {
            ...
            .then(response => {
                ...
                if ($(lastNode).attr('src') == 'images/btn_end.gif') {
					...
					while(loop) {
				        url = req_url + '?PaperDate=' + moment(art_date).format('YYYY-MM-DD') + '&page=' + lastpage;
				...
				if (lastpage > 0) {
					for (var page = 1; page <= lastpage; page++) {
				        var url = req_url + '?PaperDate=' + moment(art_date).format('YYYY-MM-DD') + '&page=' + page;
				        var response2 = syncRequest("GET", url);
		                var $2 = cheerio.load(response2.getBody());
...
//        });
    ...
    app.get('/mk', function(req, res) {
		...
		default :
			mseq = "mk";
			req_url = "http://epaper.mk.co.kr/PaperList.aspx";
			mtit = "매경신문";
			formData = {'PaperDate': moment(art_date).format('YYYY-MM-DD')};
			break;
		}
...
//        });
    ...
    app.get('/chosun', function(req, res) {
        ...
        var req_url = "https://news.chosun.com/smart/svc/latest.html?catid=mchosun&pn="
        axios.post(req_url, {
            headers: {
                "Accept-Charset" : "utf-8",
                "referer": "https://news.chosun.com/",
...
//        });
    ...
    app.get('/chosun/BreakNewsView', function(req, res) {
        ...
        var news = [];
        var req_url = "http://msvr2.chosun.com/breakingnews/todayNewsList.do";
